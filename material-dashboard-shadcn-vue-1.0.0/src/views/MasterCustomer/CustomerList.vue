...
const handleExport = async () => {
  ...
  try {
    ...
    link.download = `master-customer_${yyyy}-${mm}-${dd}.xlsx`
    document.body.appendChild(link)
    link.click()
    link.remove()
    window.setTimeout(() => URL.revokeObjectURL(url), 0)
  } catch (err: any) {
    error.value = getErrorMessage(err, 'Gagal export customer')
  } finally {
              ...
              <tr v-for="(cust, index) in filteredCustomers" :key="cust.id" class="border-t">
                ...
                <td class="px-3 py-2">
                  <div class="flex items-center gap-2">
                    ...
                    <Button size="sm" variant="outline" class="border-red-200 bg-red-600 text-white hover:bg-red-700 hover:text-white" @click="openConfirm(cust)">Hapus</Button>
                  </div>
                </td>
              </tr>
            </tbody>
